...
describe("GetCoordinatesUseCase", () => {
    ...
    it("should return an error if file name is not provided", async () => {
        await expect(getCoordinatesUseCase.execute("")).rejects.toThrow("File name is required");
    ...
    it("should return an error if file format is invalid", async () => {
        ...
        await expect(getCoordinatesUseCase.execute("invalidFile.txt")).rejects.toThrow("The coordinates file is not in a valid format");
